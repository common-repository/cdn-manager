 ...
 * CDN_FOR_WP_Backups
 *
 * Singleton class for creating backups, all scheduling is handled by cdn4WP
 */
class CDN_FOR_WP_Backups extends CDN_FOR_WP_HM_Backup {
	...
	/**
	 * Return the current instance of CDN_FOR_WP_Backups
	 *
	 * @static
	 * @access public
	 */
	public static function get_instance() {
		...
		if ( empty( self::$instance ) ) {
			self::$instance = new CDN_FOR_WP_Backups();
		}
	...
	public function __construct() {
        ...
        $options['excluded_strings'] = explode( PHP_EOL, $options['excluded_strings'] );

		$this->set_excludes( apply_filters( 'cdn4wp_backup_excludes', CDN_FOR_WP_GLOBAL_EXCLUDES ) );
		$this->set_excludes( apply_filters( 'cdn4wp_backup_excludes', $options['excluded_strings'] ), true );

			$this->set_includes( apply_filters( 'cdn4wp_backup_includes', $options['included_file_extensions'] ) );

		$this->filesize_transient = 'cdn4wp_' . '_' . $this->get_type() . '_' . substr( md5( $this->exclude_string() ), 20 ) . '_filesize';

	}

    public static function backup_request() {

							        if ( !empty($_GET['_cdn']) && esc_attr( $_GET['_cdn'] ) == 'synct' ) {
								    	...
								    	if(empty($backup) ){
	                                        ...
	                                        add_action( 'admin_footer', array( 'CDN_FOR_WP', 'do_backup_javascript' ) );
	...
	public function do_backup() {
		...
		if ( ! file_exists( $this->get_archive_filepath() ) ) {
			...
			if ( ! empty( $errors ) )
				...
				return new WP_Error( 'backup-failed', __( 'Backup file is missing.', 'cdn4wp' ) );
	...
	public function get_backup() {
		...
		if ( $status = $this->get_status() ) {
			...
			if ( $this->is_backup_still_running() )
				...
				return new WP_Error( 'backup-process-killed', __( 'Backup process failed or was killed.', 'cdn4wp' ) );
		}
		...
		if ( file_exists( $backup ) ) {
			...
			$response->path = $backupWithoutKey;
			$response->seconds_elapsed = time() - $this->start_timestamp;
			return $response;

		}

		return new WP_Error( 'backup-failed', __( 'No backup was found', 'cdn4wp' ) );
	...
	public static function get_backups() {
		...
		if ( $status = $this->get_status() ) {
 		...
 		if(is_dir($backup)){
 			...
 			if($bfiles){
 				$rfiles = array();
 				$path = get_option( 'cdn4wp_backup_path' );
	...
	public function cleanup() {
		...
		delete_option( 'cdn4wp_backup_path' );
	...
	public function get_estimate_size() {
		...
		global $cdn4wp_noauth_nonce;
		wp_remote_get( add_query_arg( array( 'action' => 'cdn4wp_calculate_backup_size', 'backup_excludes' => $this->get_excludes() ), add_query_arg( '_wpnonce', $cdn4wp_noauth_nonce, admin_url( 'admin-ajax.php' ) ) ), array( 'timeout' => 0.1, 'sslverify' => false ) );
	...
	protected function do_action( $action ) {
		...
		switch ( $action ) :
	    	...
	    	case 'hmbkp_mysqldump_started' :
	    		...
	    		$this->set_status( sprintf( __( 'Dumping Database %s', 'cdn4wp' ), '(<code>' . $this->get_mysqldump_method() . '</code>)' ) );

	    	break;

	    	case 'hmbkp_mysqldump_verify_started' :

	    		$this->set_status( sprintf( __( 'Verifying Database Dump %s', 'cdn4wp' ), '(<code>' . $this->get_mysqldump_method() . '</code>)' ) );
			...
			case 'hmbkp_archive_started' :
				...
				if ( $this->is_using_file_manifest() )
					$status = sprintf( __( '%d files remaining to archive %s', 'cdn4wp' ), $this->file_manifest_remaining, '(<code>' . $this->get_archive_method() . '</code>)' );
				else
					 $status = sprintf( __( 'Creating zip archive %s', 'cdn4wp' ), '(<code>' . $this->get_archive_method() . '</code>)' );
	    	...
	    	case 'hmbkp_archive_verify_started' :
	    		...
	    		$this->set_status( sprintf( __( 'Verifying Zip Archive %s', 'cdn4wp' ), '(<code>' . $this->get_archive_method() . '</code>)' ) );
	...
	private function path() {
		...
		if ( get_option( 'cdn4wp_backup_path' ) !== $path )
			update_option( 'cdn4wp_backup_path', $path );
		...
		if ( $is_apache && function_exists( 'insert_with_markers' ) && ! file_exists( $htaccess ) && is_writable( $path ) ) {
			...
			$contents[]	= '# ' . sprintf( __( 'This %s file ensures that other people cannot download your backup files.', 'cdn4wp' ), '.htaccess' );
			$contents[] = '';
			...
			insert_with_markers( $htaccess, __( 'CDN for WordPress Backup', 'cdn4wp' ), $contents );
 ...
 * @return array
 */
function _cdn4wp_get_backups_info() {

	$hm_backup = new CDN_FOR_WP_HM_Backup();

	return array(
		'mysqldump_path' 	=> $hm_backup->get_mysqldump_command_path(),
		'zip_path' 			=> $hm_backup->get_zip_command_path(),
		'estimated_size'	=> CDN_FOR_WP_Backups::get_instance()->get_estimate_size()
	);
 ...
 */
function cdn4wp_ajax_calculate_backup_size() {

	if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'cdn4wp_calculate_backup_size' ) )
		exit;

	CDN_FOR_WP_Backups::get_instance()->get_filesize();

	exit;
}
add_action( 'wp_ajax_nopriv_cdn4wp_calculate_backup_size', 'cdn4wp_ajax_calculate_backup_size' );
