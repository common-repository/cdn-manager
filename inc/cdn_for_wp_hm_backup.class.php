...
class CDN_FOR_WP_HM_Backup {
	...
	public function set_archive_filename( $filename ) {
		...
		if ( empty( $filename ) || ! is_string( $filename ) )
			throw new Exception( __( 'archive filename must be a non empty string', 'cdn4wp' ) );

		if ( pathinfo( $filename, PATHINFO_EXTENSION ) !== 'zip' )
			throw new Exception( __( 'invalid file extension for archive filename', 'cdn4wp' ) .  '<code>' . $filename . '</code>' );
	...
	public function set_database_dump_filename( $filename ) {
		...
		if ( empty( $filename ) || ! is_string( $filename ) )
			throw new Exception( __( 'database dump filename must be a non empty string', 'cdn4wp' ) );

		if ( pathinfo( $filename, PATHINFO_EXTENSION ) !== 'sql' )
			throw new Exception( __( 'invalid file extension for database dump filename', 'cdn4wp' ) . '<code>' . $filename . '</code>' );
	...
	public function set_root( $path ) {
    	...
    	if ( empty( $path ) || ! is_string( $path ) || ! is_dir ( $path ) )
    		throw new Exception( sprintf( __( 'Invalid root path %s must be a valid directory path', 'cdn4wp' ), '<code>' . $path . '</code>' ) );
	...
	public function set_path( $path ) {
    	...
    	if ( empty( $path ) || ! is_string( $path ) )
    		throw new Exception( sptrinf( __( 'Invalid backup path %s must be a non empty (string)', cdn4wp ), '<code>' . $path . '</code>' ) );
		...
		foreach( $this->get_files() as $file ) {
		    ...
		    if ( $includes ){
	            ...
	            foreach ( $includes as $included_string ) {
	                if ( strpos( str_ireplace( trailingslashit( $this->get_root() ), '', self::conform_dir( $file->getPathname() ) ), $included_string) !== false ) {
	                    $inc = true;
	                }
	            }
		    ...
		    if ( $exclude ){
		    	$exc = false;
	            foreach ( $exclude as $excluded_string ) {
	                if ( strpos( str_ireplace( trailingslashit( $this->get_root() ), '', self::conform_dir( $file->getPathname() ) ), $excluded_string ) !== false ) {
	                	$exc = true;
	                }
	            }
	...
	public function set_type( $type ) {
		...
		if ( ! is_string( $type ) || ! in_array( $type, array( 'file', 'database', 'complete' ) ) )
			throw new Exception( sprintf( __( 'Invalid backup type %s must be one of (string) file, database or complete', 'cdn4wp' ), '<code>' . $type . '</code>' ) );
	...
	public function pcl_zip() {
		...
		global $_cdn4wp_hmbkp_exclude_string;

		$_cdn4wp_hmbkp_exclude_string = $this->exclude_string( 'regex' );

		$archive = &$this->setup_pclzip();

		// Zip up everything
		if ( $this->get_type() !== 'database' )
			if ( ! $archive->add( $this->get_root(), PCLZIP_OPT_REMOVE_PATH, $this->get_root(), PCLZIP_CB_PRE_ADD, 'cdn4wp_hmbkp_pclzip_callback' ) )
		...
		unset( $GLOBALS['_cdn4wp_hmbkp_exclude_string'] );
	...
	public function set_includes( $includes, $append = false ) {

		if ( is_string( $includes ) )
			$includes = explode( PHP_EOL, $includes );

		if ( $append )
			$includes = array_merge( $this->includes, $includes );

		$this->includes = array_filter( array_unique( array_map( 'trim', $includes ) ) );
 ...
 * @return bool
 */
function cdn4wp_hmbkp_pclzip_callback( $event, &$file ) {
    ...
    elseif ( $_cdn4wp_hmbkp_exclude_string && preg_match( '(' . $_cdn4wp_hmbkp_exclude_string . ')', $file['stored_filename'] ) )
    	return false;
